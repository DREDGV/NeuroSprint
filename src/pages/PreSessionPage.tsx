...
function getReactionModeTip(modeId: TrainingModeId): string | null {
  ...
  if (modeId === "decision_kids") {
    return "Быстрые решения: Дети — мягкий темп и простые правила ДА/НЕТ.";
  }
  if (modeId === "decision_standard") {
    return "Быстрые решения: Стандарт — цвет, форма, число и короткий boss-раунд.";
  }
  if (modeId === "decision_pro") {
    return "Быстрые решения: Про — быстрый темп, отрицания и больше переключений.";
  }
  if (modeId === "pattern_classic") {
    return "Распознавание паттернов: Классика — 15 вопросов без таймера, фокус на точность.";
  }
  if (modeId === "pattern_timed") {
    return "Распознавание паттернов: На время — короткий раунд на темп, где важно не развалить точность из-за спешки.";
  }
  if (modeId === "pattern_progressive") {
    return "Распознавание паттернов: Прогрессивный — адаптивный режим, который по ходу серии может и поднимать, и смягчать ступень.";
  }
  if (modeId === "pattern_learning") {
    return "Распознавание паттернов: Обучение — обучающий режим с подсказками и разбором ошибок.";
  }
  if (modeId === "pattern_multi") {
    return "Распознавание паттернов: Мульти — несколько пропусков подряд в хвосте ряда, ответы нужно заполнять строго по порядку.";
  }
  if (modeId.startsWith("memory_grid_classic")) {
    return "Сетка памяти: Классика — запомните последовательность и повторите без ошибок.";
  }
  if (modeId.startsWith("memory_grid_rush")) {
    return "Сетка памяти: На скорость — пройдите максимум уровней за ограниченное время.";
  }
  if (modeId === "memory_match_classic") {
    return "Пары памяти: Классика — сначала запомните несколько опорных пар, затем собирайте поле без повторяющихся ошибок.";
  }
