...
export function PatternRecognitionSetupPage() {
  ...
  return (
    <section className="panel pattern-setup-page" data-testid="pattern-setup-page">
      <h2>Распознавание паттернов</h2>
      <p className="session-intro-text">
        Поиск закономерностей в последовательностях. Тренирует логическое мышление, способность видеть паттерны и предсказывать следующие элементы.
      </p>

      <InfoHint title="Как играть в Распознавание паттернов" testId="pattern-setup-hint">
        <p><strong>1.</strong> Смотрите на ряд сверху и ищите правило: цикл, чередование, зеркало, прогрессию или числовой шаг.</p>
        <p><strong>2.</strong> Сначала поймите ритм паттерна, а уже потом выбирайте ответ.</p>
        <p><strong>3.</strong> В multi-gap режиме заполняйте пропуски по порядку, слева направо.</p>
        <p><strong>4.</strong> В обучающем режиме используйте подсказки, в рабочих — проверяйте себя без опоры.</p>
        <hr />
        <p><strong>Что тренирует:</strong></p>
        <ul>
          <li>Логическое мышление и анализ</li>
          <li>Способность видеть закономерности</li>
          <li>Предсказание следующих элементов</li>
        </ul>
        <p><strong>Совет:</strong> Ищите повторяющиеся группы элементов. Часто паттерн строится на цикле из 2-4 элементов, который повторяется.</p>
      </InfoHint>
